...
class Visualizer(object):
    ...
    def __init__(self, render_size=1024, config=None, device='cuda:0', part='body', background=None,
        ...
        if background is not None:
            ...
            self.bg = torch.zeros([1, 1, self.render_size, self.render_size], device=self.device, dtype=torch.float32)
    ...
    def _setup_renderer(self):
        ...
        self.render = SRenderY(self.render_size, obj_filename=topology_path, uv_size=uv_size,
                               rasterizer_type=self.rasterizer_type).to(self.device)
        ...
        self.smplx_face_eye_mask = self.tex_flame2smplx(self.flame_face_eye_mask,
                                                        np.zeros([uv_size, uv_size, 1], dtype=np.float32))
        ...
        self.smplx_face_mask = self.tex_flame2smplx(self.flame_face_mask,
                                                    np.zeros([uv_size, uv_size, 1], dtype=np.float32))
    ...
    def light_albedo(self, opdict):
        ...
        coarse_normals = util.vertex_normals(opdict['vertices'],
    ...
    def render_results(self, opdict, input_images, overlay=False, light_on=False, moderator_weight=None, use_deca=False,
                       background=None):
        ...
        shape_images, normal_images, grid = self.render.render_shape(opdict['vertices'], opdict['transformed_vertices'],
                                                                     background=bg, return_grid=True, h=1024, w=1024)
        visdict = {
            'inputs': input_images,
        ...
        if moderator_weight is not None:
            ...
            color_shape_images = self.render.render_shape(opdict['vertices'], opdict['transformed_vertices'],
                                                          background=bg, colors=face_colors, h=h, w=w)
        ...
        render_ops = self.render(opdict['vertices'], opdict['transformed_vertices'], uvmap, background=bg,
                                 lights=lights, h=h, w=w)                      
