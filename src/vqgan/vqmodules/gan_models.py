...
def setup_vq_transformer(args, config, load_path=None, test=False, version=None):
    ...
    # weight_decay_strength = 0.009
    # g_optimizer = ScheduledOptim(
    #         torch.optim.AdamW(generator.parameters(),
    #                           betas=(0.9, 0.98), eps=1e-09,weight_decay=weight_decay_strength),
    #                           learning_rate,
...
def calc_vq_loss(pred, target, quant_loss, quant_loss_weight=1.0, alpha=1.0):
    ...
    # body_pose_loss = alpha * nn.L1Loss()(pred[:, :, 50:164], target[:, :, 50:164])
    
    # wrists_pose_loss = alpha * nn.L1Loss()(pred[:, :, 164:176], target[:, :, 164:176])

    # body_pose_loss = alpha * nn.L1Loss()(pred[:, :, 50:176], target[:, :, 50:176])
    ...
    # hands_pose_loss = alpha * nn.L1Loss()(pred[:, :, 179:359], target[:, :, 179:359])

    # combined_loss = (exp_loss + body_pose_loss + jaw_pose_loss + hands_pose_loss)

    # body_pose_loss = alpha * nn.L1Loss()(pred[:, :, 50:164], target[:, :, 50:164])
    
    # Combining all losses
    # combined_loss = (exp_loss + body_pose_loss + wrists_pose_loss + jaw_pose_loss + left_hand_pose_loss + right_hand_pose_loss)
    combined_loss = (exp_loss + partbody_pose_loss + neck_pose_loss + head_pose_loss + left_wrist_pose_loss + right_wrist_pose_loss + jaw_pose_loss + left_hand_pose_loss + right_hand_pose_loss )
...
class VQModelTransformer(nn.Module):
    ...
    def get_logit(self, logits, sample=False, filter_value=-float('Inf'),
        ...
        if sample_idx is None:
            # If we're sampling, then do nucleus sampling
            if sample:
                ...
                # If not sampling, then we're just taking the top index for each position in the sequence
                probs = F.softmax(logits, dim=-1)
                top_ix = torch.argmax(probs, dim=-1)  # This gets the top index (not top-k)
                return top_ix, probs
        else:
            # Other specific sampling strategies can be implemented here
            pass
...
def nucleus_sampling(self, logits, filter_value, temperature, top_p):
    ...
    # Sort logits and apply cumulative softmax for nucleus sampling
    ...
    sorted_indices_to_remove[..., 0] = 0
    # Create a boolean mask to filter out logits over the threshold
    indices_to_remove = sorted_indices[sorted_indices_to_remove]
