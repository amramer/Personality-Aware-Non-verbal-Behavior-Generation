...
class FACTModel(nn.Module):
    ...
    def __init__(self, config, mask_index=-1, quant_factor=None):
        ...
        # Conditioning MLP layer that embed personality score on larger feature size
        self.conditioning_mlp = ConditioningMLP(input_size=1, hidden_size=80, output_size=160)
        ...
        body = self.config['speaker_full_transformer_config']['body_hidden_size']
        face = self.config['speaker_full_transformer_config']['face_hidden_size']
        audio_layers = [nn.Sequential(
            nn.MaxPool1d(4))]
        self.audio_compressor = nn.Sequential(*audio_layers)
        self.audio_projector = nn.Linear(128, face*2)
        self.audio_full_pos_embedding = PositionEmbedding(
            self.config["speaker_full_transformer_config"]["sequence_length"],
            face*2)
        self.face_motion_projector = nn.Linear(53, face*2)
        self.body_motion_projector = nn.Linear(306, body*2)
        self.face_motion_pos_embedding = PositionEmbedding(
            self.config["speaker_full_transformer_config"]["sequence_length"],
            face*2)
        self.body_motion_pos_embedding = PositionEmbedding(
            self.config["speaker_full_transformer_config"]["sequence_length"],
            body*2)
        ...
        post_layers = [nn.Sequential(
            nn.Conv1d((face * 2) + (body * 2), (face * 2) + (body * 2), 5, stride=2, padding=2,
                      padding_mode='replicate'),
            nn.LeakyReLU(0.2, True),
            nn.BatchNorm1d((face * 2) + (body * 2)))]
        for _ in range(1, quant_factor):
            post_layers += [nn.Sequential(
                nn.Conv1d((face * 2) + (body * 2), (face * 2) + (body * 2), 5, stride=1, padding=2,
                          padding_mode='replicate'),
                nn.LeakyReLU(0.2, True),
                nn.BatchNorm1d((face * 2) + (body * 2)),
                #
        ...
        self.post_projector = nn.Linear((face * 2) + (body * 2), dim)
    ...
    """
        ...
        #print("input listener past: ", inputs["listener_past"].shape)
        ...
        if personality_data is not None:
            # Expand personality_data to match the shape [B, 8, 1] of MLP
            expanded_personality_data = personality_data.unsqueeze(1).expand(B, 8, -1).float().cuda()
            # process the personality data using a conditioning MLP
            personality_features = self.conditioning_mlp(expanded_personality_data)
            # Concatenate along the last dimension to get [B, 8, 800 + fc_mlp_output]
            listener_past_features = torch.cat((listener_past_features, personality_features), dim=2).float().cuda()

        # pdb.set_trace()
        ## process the speaker embedding. no masking. apply cross modal transformer
        # print("input audio features: ",inputs['audio_full'].shape)
        audio_full_features = \
            self.audio_compressor(inputs['audio_full'].permute(0, 2, 1).contiguous()) \
                .permute(0, 2, 1).contiguous()
        # print("audio features after audio_compressor: ", audio_full_features.shape)
        audio_full_features = self.audio_projector(audio_full_features)
        # print("audio features after audio_projector: ", audio_full_features.shape)
        audio_full_features = self.audio_full_pos_embedding(audio_full_features)
        # print("audio features after audio_full_pos_embedding: ", audio_full_features.shape)

        # Extracting face features (exp and jaw_pose)
        exp_features = inputs['speaker_full'][:, :, :50]  # Extracting exp features (first 50 features)
        jaw_pose_features = inputs['speaker_full'][:, :, 176:179]  # Extracting jaw_pose features (indices 176 to 178)
        face_features = torch.cat((exp_features, jaw_pose_features), dim=2)  # Concatenating exp and jaw_pose features

        # Extracting body features (all features except exp and jaw_pose)
        body_features_part1 = inputs['speaker_full'][:, :, 50:176]  # Features between exp and jaw_pose
        body_features_part2 = inputs['speaker_full'][:, :, 179:]  # Features after jaw_pose
        body_features = torch.cat((body_features_part1, body_features_part2), dim=2)  # Concatenating to get full body features

        
        body_motion_features = self.body_motion_projector(body_features)
        # print("speaker motion features after motion_projector: ",motion_full_features.shape)
        body_motion_features = self.body_motion_pos_embedding(body_motion_features)
        
        # print("input speaker motion features: ", inputs['speaker_full'].shape)
        face_motion_features = self.face_motion_projector(face_features)
        # print("speaker motion features after motion_projector: ",motion_full_features.shape)
        face_motion_features = self.face_motion_pos_embedding(face_motion_features)
        # print("speaker motion features after motion_full_pos_embedding: ",motion_full_features.shape)
        speaker_face_features = {'x_a': audio_full_features, 'x_b': face_motion_features}
        speaker_face_features = self.cm_transformer(speaker_face_features)
        # print("speaker full features after cross modal transformer: ",speaker_full_features.shape)
        
        # Concatenating cross model transformer's speaker face features and input speaker body features
        speaker_full_features = torch.cat((speaker_face_features, body_motion_features), dim=2)
        ...
        # print("speaker full features after post_compressor: ",speaker_full_features)
        speaker_full_features = self.post_projector(speaker_full_features)
        #print("speaker full features after post_projector: ",speaker_full_features.shape)
